...
export class AccessInterceptor implements NestInterceptor {
  ...
  async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<any>> {
    ...
    if (user && cooldownSeconds) {
      this.cooldownService.checkCooldown(user.id, request.route.method, request.route.path, cooldownSeconds);
    }
    ...
    if (user) {
      ...
      this.actorContextService.set({
        user,
        meta: request.meta,
        username: request.user.appwrite.name || request.user.appwrite.email || 'Unknown',
      })
    }
