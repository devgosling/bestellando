...
export async function authenticatedFetch(url: string, init?: RequestInit, autoLogout = true) : Promise<any> {
    try {
        ...
        if (!autoLogout || error.status === 400 || error.status === 404 || error.status === 403) {
            throw error;
        }
...
export const getInfiniteQueryOptions = <TData>(
    ...
    return infiniteQueryOptions({
        ...
        queryFn: ({pageParam}) => {
            ...
            if (options?.dateSpan) {
                if (options.dateSpan.from) searchParams.set('from', options.dateSpan.from.toISOString());
                if (options.dateSpan.to) searchParams.set('to', options.dateSpan.to.toISOString());
            }

            searchParams.set('page', (pageParam).toString());

            const separator = url.includes('?') ? '&' : '?';
            return (authenticatedFetch(`${url}${separator}${searchParams.toString()}`) as Promise<PaginateResponse<TData>>)
                .then(res => ({...res, meta: {...res.meta, currentPage: Number(res.meta.currentPage)}}));
        },
        initialPageParam: 1,
